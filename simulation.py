...
def input_check(parameters):
    """
    :param parameters:  the simulation parameters entered by user
    :return: flag whether parameters are valid
    """
    flag = True
    if len(parameters) < 7:
        print("num args")
        ...
        i += 1
        max_generation = globals()['max_generation'] = int(input[i])

        #check whether the parameters have valid values. set flag to false otherwise
...
class People:
    ...
    # sets number of generations that person has to wait to L
    def set_num_generations(self):
        self.num_generations = L

    # decreases number of remaining generations that person of to wait. used after every generation
    def decrease_num_generations(self):
        if self.num_generations > 0:
            self.num_generations -= 1
    # returns person's current level of doubt
    def get_doubt(self):
        return self.current_doubt
    # decreases person's level of doubt. used when person heard rumor for 2 or more neighbors.
    ...
    #return doubt to original level
    def return_doubt(self):
        self.current_doubt = self.base_doubt

    # checks if person can spread rumor again
    def can_spread_rumor(self):
        return self.num_generations == 0

    #pass rumor to person
    def pass_rumor(self):
        self.heard = True

    #checks if person heard rumor
...
def initialize(mat):
    """
    :param mat: empty matrix
    :return: initialized matrix with people
    """
    #calculate number of different people types
    ...
    #initialize S1, S2, S3,S4 people

    for S1_people in range(num_of_S1):
        # choose random location for S1 person
        location = (row, column) = random.choice(people_free_location)
        # initilize person object
        person = People(row, column, 'S1')
        # fill matrix cell with person
        mat[row][column] = person
        # remove location from people's free location
        people_free_location.remove(location)
        #add person to total people array
...
def initializePart1(mat):
    """
        :param mat: empty matrix
        :return: initialized matrix with people with intentional placement of S4 people in grids.
        """
    ...
    #initialize S4 people in rows and columns on purpose to create a grid of S4 people.
    for S4_people in range(num_of_S4):
        location = (row, column) = random.choice(people_free_location)
        if S4_line:
            if row_turn:
...
def edge_behavior(nr, nc):
    """
    :param nr: neighbors row
    :param nc: neighbors column
    :return: updated nr,nc values with wrap around behavior
    """
    if wrap_around_hor:
        nr = nr % mat_size
    else:
...
def neighbors_coordinates(row, col):
    """
    :param row: person's row
    :param col: person's column
    :return: list of person's neighbors
    """
    neighbors = []
...
def update_percentage():
    """
    updates array of percentage of people exposed to rumor in every generation
    :return:
    """
...
def show_params(screen):
    """
    show parameters on top of screen
    :param screen:
    :return:
    """
...
def show_legend1(screen, width, legend_height):
    """
    draws legend in top of simulation screen
    :param screen:
    :param width:
    :param legend_height:
    :return:
...
def show_legend2(screen, width, legend_height):
    """
    draws legend in top of last view screen after simulation ended
    :param screen:
    :param width:
    :param legend_height:
    :return:
...
def show_status(screen, width, height, legend_height):
    """
    shows current status of simulation at the bottom of the screen
    :param screen:
    :param width:
    :param height:
    :param legend_height:
...
def show_status2(screen, width, height, legend_height):
    """
    shows status of simulation at the end in the bottom of last view screen
    :param screen:
    :param width:
    :param height:
    :param legend_height:
    ...
    """
    font = pygame.font.SysFont('Arial', 21)
    text = str(len(total_people)-len(people_exposed)) + " people didn't hear"
    text_rect = pygame.Rect(10, height - legend_height / 2 + 20, 25, 25)
    status_text = font.render(text, True, black)
    status_text_rect = status_text.get_rect(center=(text_rect.right + 100, legend_height / 2 + 570))
    status_text_rect.width += 6
...
def main():
    ...
    default_params = ["0.8", "40", "10", "20", "30", "15", "150"]
    ...
    # choose random rumor starter
    starter = random.choice(total_people)
    #spread rumor to starter
    ...
    while game_running and generation < max_generation and len(rumor_spreaders) and len(
        ...
        # updated percentage of rumor exposure
        update_percentage()

        #fill screen
        screen.fill(white)
        # show legend on top of screen
        show_legend1(screen, width, legend_height)
        # show status in bottom of screen
        show_status(screen, width, height, legend_height)
        #show selected parameters of top of screen
        show_params(screen)

        # fill every cell of the matrix with colors according to status: empty, contains person,  heard rumor,...
    ...
    while game_running:
        ...
        screen.fill(white)
        show_status2(screen, width, height, legend_height)
